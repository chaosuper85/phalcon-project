define('user/partial/agent/agent', function(require, exports, module) {
...
var search_type = "agent_name";
var search_type_val = "";
var search_type_name = ['agent_name', 'name', 'mobile'];
...
function render() {
	...
	React.render(React.createElement(Select, { defaultValue: search_type, style: { width: 80 }, onChange: handleChange, placeholder: "查询类型" }, React.createElement(Option, { value: "agent_name" }, "货代名称"), React.createElement(Option, { value: "mobile" }, "手机号"), React.createElement(Option, { value: "name" }, "用户名")), document.getElementById('filter_search_type'));
...
function bindEvent() {
	...
	util.bind($detail, 'click', function () {
		...
		util.get('/api/agent/auditDetail?id=' + id).then(function (json) {
			setTimeout(function () {
				_this.innerHTML = '查看';
			}, 500);
			if (json.error_code == 0) {
				...
				if (!pop) pop = new pop_userDetail(json.data, 'agent');
				pop.showDetail(json.data, 'agent');
			} else {
