...
<div class="boxInfo-wrapper">
			...
			{%if count($box) > 0 && !empty($box[0])%}
				{%foreach $box as $i => $item%}
					<tr>
						...
						<td class="driver">{%$item.driver_info.name%}</td>
										...
										<li class="clearfix">
											...
											{%if $utype == 1%}
												{%if $box_address_details.assign_status == 2 || $box_address_details.assign_status == 100%}
													<a href="javascript:;" class="complete" data-aid="{%$box_address_details.assign_id%}" data-type="{%$box_address_details.assign_status%}">产装完成</a>
												{%else%}
													<span class="complete">{%$boxStatus[$box_address_details.assign_status]%}</span>
												{%/if%}
											{%else%}
												<span class="complete">
						...
						<td class="funcs look {%if $utype == 2%}hidden{%/if%}">
							<div class="status-box clearfix">
								<span>待产装</span>
								<a href="/order/trace?orderid={%$data.data.order_fregiht_id%}#/{%$i + 1%}" target="_blank"><i class="icon-status-message" data-boxid="{%$item.box_id%}"></i></a>
							</div>
						</td>
					</tr>
				{%/foreach%}
	...
	{%if $status < 3 && $status != 8%}
		<div class="clearfix btn-wrap export_box_info">
			<a href="/carteam/order/export_box_info?orderid={%$data.data.order_fregiht_id%}" target="_blank" id="download_box_number" class="right">导出箱号/铅封号</a>
		</div>
	{%/if%}
</div>
{%script type="text/javascript"%}
	require('www/order/widget/view/order_details/partial/box_info/box_info').init({%json_encode($box)%},'{%$data.data.order_fregiht_id%}', {%json_encode($addressInfo)%}, '{%$data.data.carteam.carTeam_id%}');
{%/script%}