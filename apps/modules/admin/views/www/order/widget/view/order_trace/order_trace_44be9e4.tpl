...
<div id="order_trace">
	...
	<div class="trace_procedure clearfix">
		<ul>
			...
			<li class="line_4 {%if isset($order_time_line[5]) && $order_time_line[5].ok === $IS_Ok%}active{%/if%}">
				<div class="line"></div>
			</li>
			<li class="step_5 {%if isset($order_time_line[5]) && $order_time_line[5].ok === $IS_Ok%}active{%/if%}">
				<dl>
					<dt>运抵</dt>
					<dd>{%if isset($order_time_line[5])%}{%$order_time_line[5].create_time%}{%/if%}</dd>
				</dl>
			</li>
		</ul>
	</div>
	...
	<div class="trace_info clearfix">
		...
		<div class="company right">
			...
			<a href="#/id={%$freight.id%}"><i class="icon-msg"></i></a>
			<a href="tel:{%$freight.contactNumber%}"><i class="icon-phone"></i></a>
		</div>
	</div>
	<!-- 提单信息结束 -->
	<!-- 物流详情开始 -->
	<div class="trace_details clearfix">
		...
		<!-- 侧边栏开始 -->
		{%widget name="www/order/widget/view/order_trace/templete/order_trace_sidebar/order_trace_sidebar.tpl"%}
		<!-- 侧边栏结束 -->

		<!-- 物流详情开始 -->
		{%widget name="www/order/widget/view/order_trace/templete/order_trace_details/order_trace_details.tpl"%}
		<!-- 物流详情结束 -->
	</div>
	<!-- 物流详情结束 -->
</div>
