...
<div id="order_details">
	...
	<div class="order-funcs clearfix">
		<span class="status left">{%$orderType[$status]%}</span>
		{%if $utype == 1 && ($status < 6)%}
	...
	{%widget name="www/order/widget/com/order_file_download/order_file_download.tpl"%}
	<div class="order-wrap dispatch-info {%if $utype == 2 && $status == 1%}hidden{%/if%}">
		<h3 class="second-title">调度信息</h3>
		{%widget name="www/order/widget/view/order_details/partial/box_info/box_info.tpl"%}
	</div>

	<div class="order-wrap order-info">
		<h3 class="second-title">船务信息<span class="edit-records right" data-orderid="{%$data.data.order_fregiht_id%}"><a href="/orderDetail/records?orderid={%$data.data.order_fregiht_id%}" target="_blank">修改纪录</a></span></h3>
		...
		<div class="order-info-wrap clearfix">
			...
			<ul>
				...
				<li><label>货物重量</label><span>{%$orderInfo.product_info.product_weight%}</span></li>
			</ul>
		</div>
...
{%script type="text/javascript"%}
	require('www/order/widget/view/order_details/order_details').init({%json_encode($orderInfo.address_info)%}, '{%$data.data.order_fregiht_id%}', '1');
{%/script%}
