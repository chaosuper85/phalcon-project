...
<ul class="table-filter clearfix">
	...
	<li>
		<a href="/api/carTeam/dumpCarTeam?begin_time={%$param.begin_time%}&end_time={%$param.end_time%}&status={%$param.audit_status%}" class="btn" role="button" title="筛选条件：当前所选时间和状态">下载车队</a>
	</li>
	<div class="clear"></div>
	<li>
...
<table class="x-table carteam-table" border="1">
	<ul class="x-table-nav">
		<a href="/carTeam/carTeams" class="item {%$active[0]%}">全部</a>
		<a href="/carTeam/carTeams?audit_status=1" class="item {%$active[1]%}">新注册</a>
		<a href="/carTeam/carTeams?audit_status=3" class="item {%$active[3]%}">待审核</a>
		<a href="/carTeam/carTeams?audit_status=2" class="item {%$active[2]%}">已驳回</a>
		<a href="/carTeam/carTeams?audit_status=4" class="item {%$active[4]%}">已认证</a>
		<a href="/carTeam/carTeams?status=2" class="item {%$active_status[2]%}">已锁定</a>
		<a href="/carTeam/carTeams?status=3" class="item {%$active_status[3]%}">已删除</a>
	</ul>
	<thead>
		<tr>
	...
	<tbody>
		{%foreach $teams as $i => $item%}
			...
			{%/if%}
				...
				<td><a href="/carTeam/drivers?id={%$item.id%}" target="_blank" class="btn style2 btn_manage">管理</a></td>
			</tr>
		{%/foreach%}
	</tbody>
</table>
...
{%script type="text/javascript"%}
	require('user/partial/carteam/carteam').init({%json_encode($param)%});
{%/script%}