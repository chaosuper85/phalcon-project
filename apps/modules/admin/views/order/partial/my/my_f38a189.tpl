...
<table class="x-table order-all-table" border="1">
	<ul class="x-table-nav">
		{%foreach $orderType as $i => $type%}
			{%if $i == 0%}
				<a href="/ordersuper/myOrder" class="item {%$active[0]%}">{%$type%}</a>
			{%else%}
				<a href="/ordersuper/myOrder?order_status={%$i%}" class="item {%$active[$i]%}">{%$type%}</a>
			{%/if%}
		{%/foreach%}
	</ul>
	<thead>
...
{%script type="text/javascript"%}
	require('order/partial/my/my').init({%json_encode($param)%});
{%/script%}
