...
{%$BOX_TYPE = ['','普货箱', '开顶箱', '框架箱', '冷藏箱','危险品', '罐箱']%}
{%$BOX_SIZE_TYPE = ['','20GP','40GP','40HQ']%}
{%$BOX_STATUS = ['', '待提箱', '待装货', '待运抵', '已落箱，待运抵', '已运抵', '交易关闭']%}
<!-- 侧边栏开始 -->
			...
			{%foreach $box_time_line as $i => $item%}
				<li class="clearfix">
					<a href="#/{%$i + 1%}">
						...
						<div class="status">{%if $item.box_status != -1 && !empty($item.box_status) && $item.box_status != 100%}{%$BOX_STATUS[$item.box_status]%}{%/if%}{%if $item.box_status == 100%}司机已装货{%/if%}</div>
