					...
					<a href="#/{%$i + 1%}">
						...
						<div class="info clearfix">
							<div class="box_type item">
								<label>箱型</label>
								<span>{%if $item.box_type && $item.box_size_type%}{%$BOX_TYPE[$item.box_type]%} {%$BOX_SIZE_TYPE[$item.box_size_type]%}{%/if%}</span>
							</div>
							...
							<div class="box_mobile item">
								<label>铅封号</label>
								<span>{%if isset($item.box_ensupe)%}{%$item.box_ensupe%}{%/if%}</span>
							</div>
						</div>
					</a>
				</li>
