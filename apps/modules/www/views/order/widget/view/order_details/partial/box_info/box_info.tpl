										...
										<li class="clearfix">
											...
											{%if $utype == 1%}
												{%if $box_address_details.assign_status == 2 || $box_address_details.assign_status == 100%}
													...
													{%if $box_address_details.assign_status == 100%}<p>司机已产装</p>{%/if%}
												{%else%}
													<span class="complete" title="{%$boxStatus[$box_address_details.assign_status]%}">{%$boxStatus[$box_address_details.assign_status]%}</span>
												{%/if%}
											{%else%}
												<span class="complete" title="{%if $box_address_details.assign_status < 0 && $box_address_details.assign_status > 7%}{%$boxStatus[$box_address_details.assign_status]%}{%else%}待产装{%/if%}">
