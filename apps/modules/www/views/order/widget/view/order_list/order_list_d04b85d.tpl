...
{%$orderType = ['全部','待确认','待分派','待提箱','待装货','待运抵','已运抵待评价','交易成功','交易关闭']%}
...
<div id="order-list" class="user-content">
	<table class="nav">
		<tr class="nav">
			<td class="{%$pageType[0]%}"><a href="/order/list">全部</a></td>
			<td class="cut"></td>
			...
			<td class="cut"></td>
			<td class="{%$pageType[4]%}"><a href="/order/list?status=4">待装货<span>({%$data.data.total_count.4%})</span></a></td>
			<td class="cut"></td>
	...
	<form id="order_search" method="GET" action="/order/list" class="order-search-form clearfix">
		<div class="select-container"><div id="searchTypeSelector"></div></div>
		<div class="order_search_wrapper">
			<input type="text" id="q" class="search-box" placeholder="{%if $usertype == 1%}请输入委托方{%else%}请输入承运方{%/if%}" autocomplete="off" name="searchValue" aria-haspopup="true" value="{%$data.data.searchValue%}"></input>
				...
				{%foreach $orders as $item%}
					...
					<tr class="item-content">
						<td>
							...
							<p class="bottom">{%$item.contactNumber%}</p>
						...
						<td>
							...
							{%if $item.isDisplayTransport%}<a href="/order/trace?orderid={%$item.orderid%}&list=1" class="blue clearfix" target="_blank">物流详情</a>{%/if%}
						</td>
						<td class="last">
							{%if $usertype == 2%}
								<!-- 货代 -->
							...
							{%else%}
								...
								{%elseif $item.status == 3%}
									<a href="/order/details?orderid={%$item.orderid%}&dispatch=1" target="_blank" class="blue clearfix">分配/调度</a>
									<a href="/order/details?orderid={%$item.orderid%}" target="_blank" class="blue clearfix">装货确认</a>
									<a href="/carteam/order/product_address_list?orderid={%$item.orderid%}" target="_blank" class="blue clearfix">查看装货单</a>
								{%elseif $item.status == 4%}
									<a href="/carteam/order/product_address_list?orderid={%$item.orderid%}" class="blue clearfix">查看装货单</a>
