...
class Logger {
     ...
     * 文件型日志类
     */
    const LOG_FILE           = "Library\Log\Log_File";
    /**
     * 计时器分隔符
     */
    ...
    private static $_log_level_map = array(
        self::LOG_LEVEL_DEBUG        => Log_Abstract::LL_DEBUG,
        self::LOG_LEVEL_ERROR        => Log_Abstract::LL_FATAL,
        self::LOG_LEVEL_FATAL        => Log_Abstract::LL_FATAL,
        self::LOG_LEVEL_INFO         => Log_Abstract::LL_TRACE,
        self::LOG_LEVEL_INTERMEDIATE => Log_Abstract::LL_TRACE,
        self::LOG_LEVEL_NOTICE       => Log_Abstract::LL_NOTICE,
        self::LOG_LEVEL_TRACE        => Log_Abstract::LL_TRACE,
        self::LOG_LEVEL_TRANSFER     => Log_Abstract::LL_TRACE,
        self::LOG_LEVEL_STATS        => Log_Abstract::LL_TRACE,
        self::LOG_LEVEL_UNION        => Log_Abstract::LL_TRACE,
        self::LOG_LEVEL_WARN         => Log_Abstract::LL_WARNING,
    	self::LOG_LEVEL_HYBRID       => Log_Abstract::LL_TRACE,

        self::LOG_LEVEL_DB           => Log_Abstract::LL_TRACE,
    );
    /**
     * 向前兼容的日志记录函数名
     */
    ...
    public static function init() {
        $argv = func_get_args();
        if (func_num_args() < 3 || !in_array($argv[0], self::$_log_class) || !@isset(Log_Abstract::$log_level_name[$argv[1]])) {
            return false;
        }
        if (!is_null(self::$_logger)) {
            return true;
